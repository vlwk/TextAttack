...
from imperceptible_experiments.datasets.perturbation_data_generator import PerturbationDatasetGenerator
...
def generate_datasets(split: str, perturbation_depth: int, base_pct: float):
    ...
    gen.generate_control_dataset(f"datasets/t5/ag_news/files/{split}/clean_full_{split}.csv")
    ...
    for p in [1, 2, 3, 4]:
        ...
        gen.generate_full_dataset(
            perturbation_allocation,
            f"datasets/t5/ag_news/files/{split}/{PERTURBATION_NAMES[p]}_full_1to{perturbation_depth}_{split}.csv"
        )
