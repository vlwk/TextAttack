...
from imperceptible_experiments.datasets.sentiment_classification.dair_ai_emotion.perturbation_data_generator import EmotionPerturbationDatasetGenerator
...
def generate_datasets(split: str, perturbation_depth: int, base_pct: float):
    ...
    gen.generate_control_dataset(f"datasets/sentiment_classification/dair_ai_emotion/oneshot/clean_full_{split}.csv")
    ...
    for p in [1, 2, 3, 4]:
        ...
        gen.generate_full_dataset(
            perturbation_allocation,
            f"datasets/sentiment_classification/dair_ai_emotion/oneshot/{PERTURBATION_NAMES[p]}_full_1to{perturbation_depth}_{split}.csv"
        )
