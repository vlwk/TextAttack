...
def parse_args():
    ...
    parser.add_argument("--dataset_name", default="max_toxic", help="Dataset name")
...
def generate_plot(base_dir, output_dir, dataset_name, num_examples, popsize, maxiter):
    ...
    for perturb in perturbations:
        ...
        for budget in budgets:
            ...
            if budget == 0:
                ...
                log_path = os.path.join(base_dir, "tc_t", subdir, "log.csv")
                if os.path.exists(log_path):
                    df = pd.read_csv(log_path)
                    # For TC-T, success is when toxic examples are classified as toxic
                    successes = (df['predicted_class'] == 1).astype(float)
                else:
    ...
    plt.title("Toxic Content Classification\nIBM Toxic Content Classifier (with 95% CI)")
    ...
    # Save plot
    plt.savefig(os.path.join(output_dir, 'max_toxic_mine.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # Generate plot
    output_dir = os.path.join("figures", "tc_t")
    ...
    print(f"Generated plot in {os.path.join(output_dir, 'max_toxic_mine.png')}")
