...
def parse_args():
    ...
    parser.add_argument("--dataset_name", default="conll2003", help="Dataset name")
...
def generate_plot(base_dir, output_dir, dataset_name, num_examples, popsize, maxiter):
    ...
    for perturb in perturbations:
        ...
        for budget in budgets:
            ...
            if budget == 0:
                ...
                log_path = os.path.join(base_dir, "ner_t", subdir, "log.csv")
                if os.path.exists(log_path):
                    df = pd.read_csv(log_path)
                    # For NER-T, success is when predicted class matches target class
    ...
    plt.title("NER Model Targeted Attacks (with 95% CI)")
    ...
    # Save plot
    plt.savefig(os.path.join(output_dir, 'ner_targeted_mine.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    # Generate plot
    output_dir = os.path.join("figures", "ner_t")
    ...
    print(f"Generated plot in {os.path.join(output_dir, 'ner_targeted_mine.png')}")
