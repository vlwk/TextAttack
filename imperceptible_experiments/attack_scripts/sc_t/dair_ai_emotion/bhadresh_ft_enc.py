...
from imperceptible_experiments.finetune_scripts.sc_t.bhadresh_ft_enc.train import WordEncoder
from imperceptible_experiments.model_wrappers.sc_t.bhadresh_ft_enc import BhadreshFtEncWrapper
...
# === SANITY CHECKS ===
assert args.perturbs_start_incl < args.perturbs_end_excl
assert args.popsize > 0 and args.maxiter > 0 and args.num_examples > 0
...
model_wrapper = BhadreshFtEncWrapper(model=model, tokenizer=tokenizer)
...
for pert in range(args.perturbs_start_incl, args.perturbs_end_excl):
    ...
    checkpoint_dir = os.path.join(
        "results/sc_t/dair_ai_emotion",
        f"num{args.num_examples}", "bhadresh_ft_enc",
        f"pop{args.popsize}_iter{args.maxiter}",
        args.perturbation_type, f"target{args.target_class}", f"pert{pert}"
    )
